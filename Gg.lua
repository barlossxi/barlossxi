...
page7:Dropdown("Select Boss", {"Boss"}, 1, function(text)
if text == "Boss" then
    function mobs()
    spawn(function()
        _G.mobs = true
...
page7:Dropdown("Select Mobs", {"Ichigo"}, 1, function(GGOP)
if GGOP == "Ichigo" then
    function kklpi()
    spawn(function()
        _G.kkso = true
        while _G.kkso do wait()
            pcall(function()
                local function GetClosestPlayer()
                local target = nil
                for i,v in pairs(workspace.Lives:GetDescendants()) do
...
page7:Toggle("Ichigo Farm", function(ASDG)
    _G.kkso = ASDG
    print('mobs: ', ASDG);
    if ASDG then
        kklpi();
...
local player = page4:Label('Z,X,C,V')
