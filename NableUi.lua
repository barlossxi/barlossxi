...
function library.Create(options)
	...
	local BlurFrame = BlurFrame(Main)
	...
	LogoHub.Size = UDim2.new(0, 35, 0, 20)
	if type(options.Logo) == 'string' and not options.Logo:find('rbxassetid://') then
	...
	UserInputService.InputBegan:Connect(function(input)
		if input.KeyCode == Enum.KeyCode.RightControl then
			if OpenUI then
				...
				BlurFrame.Enabled = false
			else
				OpenUI = true
				Shadown.Visible = true
				BlurFrame.Enabled = true
			end
		end
	end)
	...
	library.TapGroup.CreateTap = function(options)
		...
		library.Sections = {}
		library.Sections.CreatePage = function()
