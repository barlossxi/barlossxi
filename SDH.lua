...
local DropdownMONS = Tabs.Main:AddDropdown("DropdownMONS", {
        Title = "Select Npc",
        Values = MONS,
        Multi = false,
        Default = false,
        Callback = function(reerrtytjj)
        Select = reerrtytjj
        
        end
    })
    
    local ToggleAutoNp = Tabs.Bossz:AddToggle("ToggleAutoNp", {Title = "Auto NPC", Default = false })
ToggleAutoNp:OnChanged(function(wyzbb)
_G.AutoNpc = wyzbb
    while _G.AutoFarm do wait()
game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Workspace").Monster.Mon[Select].HumanoidRootPart.CFrame * CFrame.new(0,0,0)
end
end)
